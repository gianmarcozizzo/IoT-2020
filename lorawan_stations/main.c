...
/* ATTENTION: this code is a modified version of the original code that you can find in the RIOT repository
   at https://github.com/RIOT-OS/RIOT/tree/master/tests/pkg_semtech-loramac
   For this reasone I will not comment in detail the default code portion
*/
...
static int cmd_start(int argc, char **argv){                                    // function that starts the process
  if (argc < 2) {                                                               // arg[0]: start
      printf("usage: %s <ID>\n", argv[0]);                                      // arg[1]: station ID
      return 1;
  }

  char json[128];                                                               // initializing data that will be published
  char* id = argv[1];                                                           // needed to avoid an error that I had passing directly argv[1] to random_values() in line 97

  while(1){

    random_values(json, id);                                                    // generates random values

    puts(json);                                                                 // writes on the stdout

                                                                                // further about semtech_loramac_send() info in the RIOT Documentation https://riot-os.org/api/semtech__loramac_8h.html
  ...
  xtimer_sleep(120);                                                            // high value to avoid duty cicle restrictions
...
// In the following section (default) you can find command's usage utility
static void _loramac_usage(void)
{
    puts("Usage: loramac <start|get|set|join|tx|link_check"                     // added the start usage
...
// In the following section (default) you can find the setting of the basic commands
...
static const shell_command_t shell_commands[] = {
    ...
    { "start", "start the process", cmd_start},                                 //  added the command cmd_start
    { NULL, NULL, NULL }
};
