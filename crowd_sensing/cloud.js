...
async function displayResult(){
  ...
  var mins = ('0'+today.getMinutes()).slice(-2);                                // it's awful that I have to do this for having a 2 digits
  var secs = ('0'+today.getSeconds()).slice(-2);
  var time = hurs + ":" + mins + ":" + secs;
  var dateTime = date+' '+time;                                                 // current datetime

  result = movementDetection();                                                 // displaying datetime and computed value on the HTML app
  var activity = '';                                                            // the expected status is just for tesing the cloud anaylsis
  treshold = Math.abs(result - 9.81);
  if (treshold > 0.15) {
    stco.style.background = 'springgreen';
    dact.innerHTML = "<b> Expected status: </b>" + "moving" + "<br/>" + "<b> Computed value: </b>" + treshold.toFixed(3) + "<br/>" + "<b> Time: </b>" + dateTime;
    activity = 'moving';
  } else {
    stco.style.background = 'tomato';
    dact.innerHTML = "<b> Expected status: </b>" + "stopped" + "<br/>" + "<b> Computed value: </b>" + treshold.toFixed(3) + "<br/>" + "<b> Time: </b>" + dateTime;
    activity = 'stopped';
    }
